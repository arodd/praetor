...
**While a performance is running, only `/pause`, `/resume`, `/stop`, and `/next` (typed or via Alt+X) are accepted.** Anything else you type is rejected with a toast and never reaches the game. `/play` itself also requires an active connection to start, and refuses to start — with a message explaining why — if a Lua automation mode other than `disable` is currently running, since its queued commands would interleave with the performance; stop it first (Alt+X, or `/mode disable`).
